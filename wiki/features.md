...
### representation（本文表現）について

* `representation` は「どの本文表現を優先して返すか」を指定するパラメータです（厳密指定ではありません）。
* 指定した表現が取得できない場合は、ツール内部で自動的にフォールバックして本文を返します。
  * Cloud: `storage` → `view` → `export_view`
  * オンプレ: `storage` → `view`
...
現時点では、以下の操作には対応していません。

- **ページ内のインラインコメント** を取得すること  
- **ページに添付されたファイル** を取得すること

...
- 🧾 **結果データの整形**\
  上記の検索結果やページ内容は、構造化データ（JSON）として提供されるとともに、読みやすい Markdown 形式のテキストでも出力されます。例えば検索結果ではページ一覧を Markdown のリストとして整形し、ページ本文取得では HTML 本文をコードブロック内に埋め込んだ Markdown を生成します。
